...
class NanoDetHeadRFCUDN(NanoDetPlusHead):
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        super(NanoDetHeadRFCUDN, self).__init__(
            num_classes,
            loss,
            input_channel,
            feat_channels,
    ...
    def _get_loss_from_assign(self, cls_preds, reg_preds, decoded_bboxes, decoded_bboxes_refine, assign, epoch):
        ...
        if len(pos_inds) > 0:
            ...
            loss_dn = self.loss_dn(
                decoded_bboxes_refine[pos_inds]/640,
                bbox_targets[pos_inds]/640,
            )

            assert not torch.isnan(loss_dn).sum()>0 , 'loss_dn is nan'
            assert not torch.isinf(loss_dn).sum()>0 , 'loss_dn is inf'
        ...
        loss = loss_qfl + loss_bbox + loss_dfl + loss_bbox_refine + loss_bbox_curr + loss_dn
        loss_states = dict(loss_qfl=loss_qfl, loss_bbox=loss_bbox, loss_dfl=loss_dfl, loss_bbox_refine=loss_bbox_refine,loss_bbox_curr = loss_bbox_curr,loss_dn= loss_dn)
        return loss, loss_states
